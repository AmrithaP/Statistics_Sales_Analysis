...
bag <- sample(bag, 9, replace = FALSE) # No replacement = 
bag
...
```{r}
lt3 <- c(1, 2) # Event lt3 = rolls less than 3
lt3 
even_roll
union(lt3, even_roll) # JOINT event - as they have common elemnents, here 2
```
...
n <- 8
k <- 0
zero_even <- choose(n,k)
zero_even
atleast_two_even <- 1- ((one_even * zero_even)/6**8)
atleast_two_even
```
...
flip_coin <- function (n) {
  flips <- sample(list("h","t"), n, replace=T)
  flips
}
flip_coin(6)


# out of `n` rolls (for 1 or more `n`s), log proportion that are h
flip_coin_head <- function (n) {
  # proportion of `x` rolls that result in a 6
  f <- function (x) (sum(flip_coin(x) == "h") / x)
...
# build a quick dataframe to collect our MC Simulation
flips <- seq(1, 10000, length.out = 1000)
prop_head <- flip_coin_head(flips)

df <- data.frame(flips, prop_head)

ggplot(data = df) +
  geom_hline(yintercept = 1/2, color = "orange") +
  geom_line(mapping = aes(x = flips, y = prop_head),
            color = "darkblue") +
  scale_y_continuous(breaks = c(0, 1/2, 1),
                     labels = c("0", "1/2", "1"),
                     limits = c(0, 1)) +
  labs(x = "Number of coin flips",
       y = "Proportion Resulting in h",
       title = "Proportion of head-flips for Increasing # of Trials") +
  theme_classic()
```
