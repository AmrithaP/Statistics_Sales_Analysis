...
1.  Define and explain a (numeric) random variable that captures this phenomenon. Catching a fish : 1 Not catching a s fish : 0

2.  Try to come up with a framework that describes probabilities for all possible outcomes of the random variable. ANS :

3.  x=catching a fish or not , with time beinf indefinite time, summation over them would give the probability within that entire time frame

4.  t= every hr, how many fishes I catch in that hour?

5.  t= some hr, how mnay fishes have I caught so far?

*Hint: different situations might call for adjustments to the framework.*
...
```{r}
ggplot() +87999888888888888889999999999999999999999999999999999999999999999999999999999999999999999999ppppppppppppppppppppppppppppppppppppppppppppppppp7pp
...
Now we can see a *roughly* exponential decay, aligning with our notion for the Exponential Distribution. In a way, this makes sense since (under the circmstances) we might expect that a random house in Ames is more likely to be built recently than a long time ago.
...
To generalize the normal distribution for any $\mu$ and any $\sigma$, we use a "z-score". This converts any normal distribution to the **Standard Normal Distribution** with a mean of 0 and a standard deviation of 1.
